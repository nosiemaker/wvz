...
export default function TripsPage() {
  ...
  const loadData = async () => {
    try {
      const [active, historyData, currentUser] = await Promise.all([
        getActiveTrips(),
        getAllTrips(),
        getCurrentUser()
      ])
      ...
      const canSelfDrive = !!currentUser?.profile?.can_drive
      setCanDrive(canSelfDrive)
      if (canSelfDrive) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "approved":
        return { bg: "bg-blue-500", text: "text-white", label: "Approved" }
      case "pending_supervisor":
        return { bg: "bg-orange-500", text: "text-white", label: "Pending Supervisor" }
      case "pending_allocation":
        return { bg: "bg-purple-500", text: "text-white", label: "Pending Allocation" }
      default:
          ...
          <div className="bg-gradient-to-br from-[#3E2723] to-[#5D4037] rounded-[28px] p-6 shadow-xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button
                  onClick={() => router.push("/mobile/inspections/post-trip?tripId=" + activeTrip.id)}
        ...
        {canDrive && (
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-4 mt-4">
              ...
              <h3 className="text-[12px] font-black text-slate-400 uppercase tracking-[2px]">
                My Requests
              </h3>
            </div>
            <div className="space-y-3">
              {requests.length > 0 ? (
                requests.map((request) => {
                  if (!["pending_supervisor", "pending_allocation", "approved", "completed"].includes(request.status)) {
                    return null
                  }
                  const status = getStatusBadge(request.status)
                  const existingTrip = request.trips && request.trips.length > 0 ? request.trips[0] : null
                  const canStart = request.status === "approved" && request.is_self_drive

                  return (
                    <div
                      key={request.id}
                      className="bg-white rounded-[24px] border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] p-5 space-y-4"
                    >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] font-bold text-slate-400">
                              {request.vehicles?.registration || "No vehicle yet"}
                            </p>
                          </div>
                        </div>
                        <span
                      ...
                      <div className="flex items-center gap-4 text-[11px] font-black text-slate-400">
                        ...
                        <div className="flex items-center gap-1.5">
                          <MapPin size={12} />
                          {request.start_location || "Office"}
                        </div>
                      </div>

                      <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-50">
                        {existingTrip && existingTrip.status === "active" && (
                          <button
                            onClick={() => router.push("/mobile/trips")}
                            className="px-4 py-2 rounded-xl bg-blue-500/10 text-blue-700 text-[12px] font-black uppercase tracking-widest"
                          >
                            Manage Trip
                          </button>
                        )}
                        {existingTrip && existingTrip.status === "completed" && (
                          <button
                            onClick={() => router.push("/mobile/trip-details?id=" + existingTrip.id)}
                            className="px-4 py-2 rounded-xl bg-green-500/10 text-green-700 text-[12px] font-black uppercase tracking-widest"
                          >
                            View Trip
                          </button>
                        )}
                        {!existingTrip && canStart && (
                          <button
                            onClick={() => {
                            ...
                            className="px-4 py-2 rounded-xl bg-[#EE401D] text-white text-[12px] font-black uppercase tracking-widest"
                          >
                            Start Trip
                          </button>
                        )}
                        {!existingTrip && !canStart && (
                          <span className="text-[11px] font-bold text-slate-400">Awaiting approval</span>
                        )}
                      </div>
                    </div>
                  )
                ...
                <div className="py-10 text-center bg-slate-50/50 rounded-[24px] border border-dashed border-slate-100">
                  <p className="text-slate-400 font-black text-[12px] uppercase tracking-[2px]">No requests yet</p>
                </div>
              )}
            </div>
          </div>
